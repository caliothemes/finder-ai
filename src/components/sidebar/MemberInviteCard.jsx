...
export default function MemberInviteCard() {
  ...
  return (
    <div 
      className="relative rounded-xl p-4 overflow-hidden shadow-sm"
      style={{
        background: isDark 
          ? 'linear-gradient(to bottom, rgba(147, 51, 234, 0.2), rgba(147, 51, 234, 0.1))' 
          : 'linear-gradient(to bottom, #f3e8ff, #e9d5ff)',
        border: `1px solid ${isDark ? 'rgba(147, 51, 234, 0.3)' : '#d8b4fe'}`
      }}
    >
      {/* Background texture */}
      ...
      <div className="relative">
        {/* Header */}
        <h3 className="font-bold text-sm mb-3" style={{ color: isDark ? 'var(--text-primary)' : '#0f172a' }}>
        ...
        <ul className="space-y-2 mb-4">
          {benefits.map((benefit, idx) => (
            <li key={idx} className="flex items-center gap-2 text-xs" style={{ color: isDark ? 'var(--text-secondary)' : '#334155' }}>
        ...
        <p className="text-xs font-semibold text-center mt-2" style={{ color: isDark ? '#a78bfa' : '#7c3aed' }}>
